...
function serverWriteHandler (cn, req, rsp) {
    var pathObj = cutils.getSoKeyObj(req.url),
        target = cn._target(pathObj.oid, pathObj.iid, pathObj.rid),
        value = cutils.decodeJsonObj(req.url, req.payload);
...
function serverCreateHandler (cn, req, rsp) {
    var pathObj = cutils.getSoKeyObj(req.url),
        ...
        if (req.payload.length === 0) {
            ...
            var obj = {};
            _.forEach(req.payload["resources"], function (resrc) {
                obj[resrc["id"]] = resrc["value"];
            });
        }
    ...
    if (!target.exist) {
        cn.createInst(pathObj.oid, pathObj.iid, obj, createCallback);
    } else  {
        rsp.code = RSP.badreq;
        rsp.end();
    }

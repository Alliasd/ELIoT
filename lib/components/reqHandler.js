...
function serverReadHandler (cn, req, rsp) {
    ...
    if (!target.exist) {
        ...
        cn.so.dump(pathObj.oid, { restrict: false }, readCallback);
        ...
        cn.so.read(pathObj.oid, pathObj.iid, pathObj.rid, { restrict: false }, readCallback);
    }
}
...
function serverCreateHandler (cn, req, rsp) {
    ...
    if (!target.exist || !iid) {
        ...
        cn.createInst(pathObj.oid, iid, value, createCallback);
    }
}
...
function serverDeleteHandler (cn, req, rsp) {
    ...
    if (JSON.stringify(pathObj) != '{}') {
        var oid = pathObj.oid;
        var iid = pathObj.iid;
        var obj = cn.getSmartObject();
        delete obj[oid][iid];
    } 

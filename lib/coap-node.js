        ...
        if (err) {
            network.get_interfaces_list(function(err, list) {
                if (err) {
                    ...
                    self.so.connMonitor[0].routeIp = 'unknown';
                    callback(err);
                } else {
                    self.ip = list[0].ip_address;
                    self.mac = list[0].mac_address;
                    self.so.set('connMonitor', 0, 'ip', self.ip);
                    self.so.set('connMonitor', 0, 'routeIp', list[0].gateway_ip || 'unknown');
                    callback(null, { ip: list[0].ip_address, mac: list[0].mac_address, routeIp: list[0].gateway_ip});
                }
            });
...
CoapNode.prototype.createInst = function (oid, iid, resrcs, callback) {
  ...
  self.so.init(oid, iid, resrcs);
  callback(null, resrcs);
};
...
CoapNode.prototype.lookup = function (clientName, callback) {
    var reqObj = {
            hostname: '224.0.1.187',
            port: '5683', //this._serverPort,
        ...
        if (req.multicast !== true) {
            startMultiListener(self, function (err) {
                if (err) {
                    ...
                    invokeCbNextTick(null, callback);
                }
            });
        }
    });
...
CoapNode.prototype._getAttrs = function (oid, iid, rid) {
    ...
    defaultAttrs = {
        pmin: 10, //this.so.get('lwm2mServer', 0, 'defaultMinPeriod'),
        pmax: 60, //this.so.get('lwm2mServer', 0, 'defaultMaxPeriod'),
        mute: true,
        enable: false,
        lastRpVal: null
    };
...
CoapNode.prototype._enableReport = function (oid, iid, rid, rsp, callback) {
    ...
    function reporterWrite(val) {
        ...
        if (_.isObject(val)) {
            ...
            var ridNum = cutils.ridNumber(oid,rid);
            var path = '/'+oid+'/'+iid+'/'+ridNum;
            rAttrs.lastRpVal = val;
            val = cutils.encodeJsonObj(path, rAttrs.lastRpVal);
        }
...
//coap.registerFormat('application/vnd.oma.lwm2m+json', 11543);
/*********************************************************
 * Private function                                      *
 *********************************************************/
function startListener(cn, callback) {
    ...
    coap.registerFormat('application/json', 1543);
